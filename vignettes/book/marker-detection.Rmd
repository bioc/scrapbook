...
```{r marker-violins-pbmc, fig.width=8, fig.height=12, fig.cap=paste0("Distribution of log-expression values for the top marker genes of cluster ", chosen.cluster, " in the PBMC dataset.")}
library(scater)
...
```{r, echo=FALSE}
# If they're monocytes, we should see higher CD14 than CD16.
stopifnot(which("CD14" == chosen.markers.10x$Symbol) < 50)
stopifnot(which("FCGR3A" == chosen.markers.10x$Symbol) > 500)
stopifnot(all(c("LYZ", "S100A9", "S100A8") %in% head(chosen.markers.10x$Symbol, 10)))
```
...
```{r marker-detected-pbmc, fig.width=8, fig.height=12, fig.cap=paste0("Distribution of log-expression values for the top marker genes of cluster ", chosen.cluster, " in the PBMC dataset, ranked by the mean delta-detected.")}
...
```{r marker-tsne-pbmc, fig.width=12, fig.height=5, fig.cap=paste0("$t$-SNE plot of the cells in the PBMC dataset, colored by the assigned cluster (top) or the log-expression of the top marker gene in cluster ", chosen.cluster, " (bottom).")}
...
```{r marker-heat-pbmc, fig.height=8, fig.width=8, fig.cap=paste0("Heatmap of the top markers for cluster ", chosen.cluster, " in the PBMC dataset. Each row represents a gene and each column represents a cell. Each entry is colored by the log-fold change for each cell from the mean log-expression for that gene.")}
...
```{r marker-dot-pbmc, fig.height=8, fig.width=8, fig.cap=paste0("Dot plot of the top markers for cluster ", chosen.cluster, " in the PBMC dataset. The size of each point represents the number of cells that express each gene in each cluster, while the color of each point represents the log-fold change between the cluster and the average across all clusters.")}
...
To find cluster-specific markers, we set `block=` to instruct `scoreMarkers.se()` to perform the pairwise comparisons separately in each block, i.e., plate.
Specifically, for a comparison between two clusters, we compute one effect size per plate based only on the cells in that plate.
...
The per-plate effect sizes are then averaged across plates, using a weighted mean that accounts for the number of cells involved in the comparison in each plate.
This yields a single average effect size per comparison, which we combine across comparisons into a variety of summary statistics for marker ranking. 
The mean log-expression and proportion of detected cells are similarly averaged across plates. 
...
```{r markers-violin-block, fig.width=8, fig.height=5, fig.cap=paste0("Distribution of expression values for the top-ranked marker gene (", rownames(markers.tropho[["1"]])[1], ") of cluster 1 in the trophoblast dataset. Distributions are shown for each cluster (x-axis) in each plate (panel).")}
plotExpression(
    sce.nn.tropho,
...
If we really want to enforce consistent DE across blocks, we can ask `scoreMarkers.se()` to compute a quantile instead of a weighted mean when averaging across blocks.

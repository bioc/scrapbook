...
(ii) match them up to each other for further analyses like those in Chapter \@ref(multi-condition-analysis)).
...
We recommend verifying any important conclusions^[
As opposed to other conclusions that are just there to fill up the manuscript.
I mean... does the world really need more gene regulatory networks?] by repeating the analysis without correction,
e.g., by analyzing individual batches separately to check that a putative cell type is not an artifact.
...
Admittedly, at this point, we're misusing the word "batch"^[
...
This, in turn, allows us test for differences in expression or abundance of the same cell type/state between conditions (Chapter \@ref(multi-condition-analysis)).
...
Any differences between conditions will still be preserved in the results of Chapter \@ref(multi-condition-analysis).
...
Let's demonstrate on two pancreas datasets [@grun2016denovo;@muraro2016singlecell] that we'll treat as separate batches.
Without any batch correction, we observe strong separation between these two datasets in Figure \@ref(batch-pancreas-uncorrected).

```{r batch-pancreas-uncorrected, fig.cap="$t$-SNE plot of the Grun and Muraro pancreas datasets without batch correction. Each point is a cell, colored according to its assigned batch."}
library(scRNAseq)
...
# Visualizing the data prior to MNN correction.
sce.unc.pancreas <- runAllNeighborSteps.se(sce.pca.pancreas, reddim.type="PCA")
library(scater)
plotReducedDim(sce.unc.pancreas, "TSNE", colour_by="batch")
```

```{r, echo=FALSE}
cluster.unc.pancreas <- countGroupsByBlock(
    sce.unc.pancreas$clusters,
    sce.unc.pancreas$batch,
    normalize.groups=TRUE,
    normalize.block=TRUE
)
# Check that there are indeed batch-specific clusters.
stopifnot(any(cluster.unc.pancreas == 0))
```
...
```{r, echo=FALSE}
# Check that there are indeed contributions from both batches to each cluster.
stopifnot(all(cluster.batch.mnn.pancreas > 0.01))
```
...
However, if we relied on the corrected expression values, we would draw misleading conclusions about the behavior of _INS-IGF2_ across batches.
...
```{r, echo=FALSE}
# Check that the behavior descibed in the text above is actually occurring.
in.grun <- sce.nn.mnn.pancreas$batch == "grun"
in.muraro <- sce.nn.mnn.pancreas$batch == "muraro"

original.values <- logcounts(sce.nn.mnn.pancreas)[current.insigf2,]
original.grun.clusters <- split(original.values[in.grun], sce.nn.mnn.pancreas$clusters[in.grun])
original.mean.grun <- vapply(original.grun.clusters, mean, 0)
stopifnot(max(original.mean.grun) - median(original.mean.grun) > 4)

original.muraro.clusters <- split(original.values[in.muraro], sce.nn.mnn.pancreas$clusters[in.muraro])
original.mean.muraro <- vapply(original.muraro.clusters, mean, 0)
original.sd.muraro <- vapply(original.muraro.clusters, sd, 0)
stopifnot(max(original.mean.muraro) - median(original.mean.muraro) < 1)

uncorrected.grun.clusters <- split(lowrank.unc.insigf2[in.grun], sce.nn.mnn.pancreas$clusters[in.grun])
uncorrected.mean.grun <- vapply(uncorrected.grun.clusters, mean, 0)
uncorrected.sd.grun <- vapply(uncorrected.grun.clusters, sd, 0)
stopifnot(max(uncorrected.mean.grun) - median(uncorrected.mean.grun) > 4)

uncorrected.muraro.clusters <- split(lowrank.unc.insigf2[in.muraro], sce.nn.mnn.pancreas$clusters[in.muraro])
uncorrected.mean.muraro <- vapply(uncorrected.muraro.clusters, mean, 0)
uncorrected.sd.muraro <- vapply(uncorrected.muraro.clusters, sd, 0)
stopifnot(max(uncorrected.mean.muraro) - median(uncorrected.mean.muraro) < 2)

corrected.grun.clusters <- split(lowrank.mnn.insigf2[in.grun], sce.nn.mnn.pancreas$clusters[in.grun])
corrected.mean.grun <- vapply(corrected.grun.clusters, mean, 0)
corrected.sd.grun <- vapply(corrected.grun.clusters, sd, 0)
stopifnot(max(corrected.mean.grun) - median(corrected.mean.grun) < 2)

corrected.muraro.clusters <- split(lowrank.mnn.insigf2[in.muraro], sce.nn.mnn.pancreas$clusters[in.muraro])
corrected.mean.muraro <- vapply(corrected.muraro.clusters, mean, 0)
corrected.sd.muraro <- vapply(corrected.muraro.clusters, sd, 0)
stopifnot(max(corrected.mean.muraro) - median(corrected.mean.muraro) < 2)
```
